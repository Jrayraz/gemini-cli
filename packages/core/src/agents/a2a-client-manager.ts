...
export class A2AClientManager {
  ...
  async loadAgent(
    ...
    // Wrap with custom adapter for Vertex Reasoning Engine compatibility
    // This includes URL rewriting, dialect mapping, and response unwrapping.
    fetchImpl = createVertexAdapterFetch(fetchImpl);
 ...
 */
export function createVertexAdapterFetch(
  ...
  return async (
    ...
    // 3. Dialect Mapping (Response)
    if (response.ok) {
      try {
        const responseData = await response.clone().json();

        // Unwrap { task: ... } or { message: ... } wrappers
        const result =
        ...
        if (result?.parts && Array.isArray(result.parts)) {
          for (const part of result.parts) {
            if (!part.kind) {
              if (part.text) part.kind = 'text';
              else if (part.data) part.kind = 'data';
              else if (part.file) part.kind = 'file';
            }
          }
        }
